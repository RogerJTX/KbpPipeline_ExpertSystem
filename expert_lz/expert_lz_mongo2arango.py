...
class KbpPipeline(object):
    def __init__(self):
        ...
        self.kb_expert_lz_mongo = self.mongo_con[self.config.get("mongo","kb_arango")][self.config.get("mongo","kb_expert_lz")]
        ...
        self.kb_expert_lz_arango = self.arango_con[self.config.get("arango","db")][self.config.get("arango","kb_expert_lz")]
    ...
    def query_process(self, process_date):
        ...
        new_g_table = self.kb_expert_lz_mongo.with_options(codec_options = bson.CodecOptions(unicode_decode_error_handler="ignore")) 
    ...
    def process(self, process_date, end_date=""):
        ...
        if self.total > 0:
            ...
            self.total = 0
            sum_block = 0
        logger.info("[{}]-[{}]，MONGO专家构建库查到待处理数据[{}]个,分为{}块处理".format(self.process_date, self.end_process_date, self.total,sum_block))
        ...
        for block in range(sum_block):
            ...
            for expert in experts:
                ...
                doc_list.append(kf)
                if doc_list and len(doc_list)%100==0:
                    self.kb_expert_lz_arango.bulkSave(doc_list,onDuplicate="update")
                    doc_list = []
                    logger.info("清洗库前[{}]条专家数据导入专家知识库处理完成, 更新[{}]条".format(count,self.count_graph_update))
        ...
        logger.info("日期[{}]专家MONGO构建库共找到数据[{}]条，arango专家库更新数据[{}]条".format(
            self.process_date, self.total, self.count_graph_update))

if __name__=="__main__":

    # 专家最早同步日期为 2020-10-10
