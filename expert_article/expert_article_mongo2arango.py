...
# Description  : 专家论文实体mongo -- arango同步
#******************************************************************************
...
class KbpPipeline(object):
    def __init__(self):
        ...
        self.kb_expert_lz_article_mongo = self.mongo_con[self.config.get("mongo","kb_arango")][self.config.get("mongo","kb_expert_article")]
        ...
        self.kb_expert_lz_article_arango = self.arango_con[self.config.get("arango","db")][self.config.get("arango","kb_expert_lz_article")]
    ...
    def query_process(self, process_date):
        ...
        new_g_table = self.kb_expert_lz_article_mongo.with_options(codec_options = bson.CodecOptions(unicode_decode_error_handler="ignore")) 
    ...
    def process(self, process_date, end_date=""):
        ...
        for block in range(sum_block):
            ...
            articles = list(new_g_table.find(mg_cmd_str).sort([("_id",1)]).limit(1000))
            if articles:
            ...
            for article in articles:
                ...
                if self.count_graph_update %100==0:
                    self.kb_expert_lz_article_arango.bulkSave(doc_list,onDuplicate="update")
                    doc_list = []
                    logger.info("MONGO论文构建库前[{}]条数据同步Arango完成, 更新[{}]条".format(count,self.count_graph_update))

        if doc_list:                       
            self.kb_expert_lz_article_arango.bulkSave(doc_list,onDuplicate="update")

        self.close_connection() 
        logger.info("日期[{}]MONGO论文构建库共找到数据[{}]条，arango更新数据[{}]条".format(
            self.process_date, self.total, self.count_graph_update))

if __name__=="__main__":

    # 论文最早同步日期为 2020-10-03
