...
# Filename     : expert_article_clean.py
# Description  : 专家成果论文信息清洗
#******************************************************************************
...
class ArticleClean(object):
    ...
    def __init__(self):
        ...
        self.res_kb_expert_article = self.mongo_con[self.config.get("mongo","res_kb_db")][self.config.get("mongo","res_kb_expert_resource")] 
    ...
    def process_journal(self, doc):
        '''期刊名处理：会议论文期刊名表示为会议名称'''
        journal = ""
        if doc["resourceCode"] == "J":
            journal = doc["journal"].strip() if doc["journal"] else ""
        elif doc["resourceCode"] == "C":
            journal = doc["conferenceName"].strip() if doc["conferenceName"] else ""
    ...
    def insert_batch(self, batch_data):
        ...
        dupl_datas = self.res_kb_process_expert_article.find({"resource_id":{"$in":public_codes}})
        ...
        insert_res = self.res_kb_process_expert_article.insert_many(new_batch)
    ...
    def query_daily_data(self, crawl_date):
        ...
        res = self.res_kb_expert_article.find({"resourceCode": {"$in": ["J", "C", "D"]}, "crawl_time": {"$gte": iso_date}}).sort([("_id",1)])
        return res
    ...
    def process(self, crawl_date):
        ...
        for doc in tqdm(docs):           
            ...
            clean_article = {
                ...
                "doi": self.process_str(doc["doi"]),
                "issn": self.process_str(doc["issn"]),
                "url": self.process_str(doc["url"]),
                "journal": self.process_journal(doc),
            ...
            authors = self.process_authors(clean_article, doc)
            clean_article.update({"authors": authors})

            batch_data.append(clean_article)
            count += 1
            

            # MongoDB批量写入
            if batch_data and len(batch_data)%100==0:
                logger.info("正在写入前[{}]条论文信息，论文ID RANGE=[{} - {}]".format(count, batch_data[0]["_id"], batch_data[-1]["_id"]))
        ...
        logging.info("[{}]论文数据清洗完毕，共找到论文[{}]条，跳过无论文号的数据[{}]条，批内去重[{}]条，已存在数据[{}]条，清洗库入库[{}]条".format(
