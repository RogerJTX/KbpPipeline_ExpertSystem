...
# Filename     : expert_article_kId_clean.py
# Description  : 论文共同作者添加KID
# ******************************************************************************
import logging
import requests
...
def set_log():
    logging.basicConfig(level=logging.INFO)
    file_log_handler = RotatingFileHandler(os.path.join(dir_path, "expert_article_kid_log.txt"),
...
class ArticleClean(object):
    ...
    def __init__(self):
        ...
        self.res_kb_process_expert_article = self.mongo_con[self.config.get("mongo", "res_kb_process")][
            self.config.get("mongo", "process_expert_article")]  # 论文清洗库
    ...
    def process_kId(self, author):
        ...
        # 查找库中已有数据
        exist_data = self.res_kb_process_expert_article.find_one({"authors": {
    ...
    def query_daily_data(self, crawl_date):
        ...
        res = self.res_kb_process_expert_article.find({"crawl_time": {"$gte": iso_date}, "authors.kId": ""}).sort(
            [("_id", 1)])
        return res
    ...
    def process(self, crawl_date):
        ...
        for doc in tqdm(docs):
            ...
            result = self.res_kb_process_expert_article.update_one({"_id": doc["_id"]},
                                                                  {"$set": update})
            self.count_update += result.modified_count

        logging.info("[{}]论文数据KID添加完毕，共找到论文[{}]条，更新[{}]条".format(
