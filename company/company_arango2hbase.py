...
# Filename     : company_arango2hbase.py
# Description  : 公司实体导入；从分区中读取数据调用pipeline处理，并存入HBASE结构化文档库；每天跑数据
#******************************************************************************
...
def set_log():
    ...
    file_log_handler = RotatingFileHandler(os.path.join(Log_path,"com_hbase2es.log"), maxBytes=1024 * 1024 * 300, backupCount=10)
...
class EntityPipeline(object):
    ...
    def process_company_hbase_data(self,item):
        ''' 将读取的arango记录转换成hbase存储结构'''

        classid = self.news_class_ids['企业']
        ...
        column_family = {
            b"info:alter_names": bytes(json.dumps(item["alter_names"], ensure_ascii=False), encoding="utf8"),
            ...
            #b"info:financing_round": bytes(item["financing_round"], encoding="utf8"),
    ...
    def link_arangoDb(self, date_str):
        ...
        logger.info("Arango库共含有[公司] %s条记录，本次查询%s条记录！" % (collection.figures()['count'],len(query)))
        num = 0
        num_ins = 0
        pre_send_rowkey = []
        pre_send_data = []
        ...
        for item in query:
            num += 1
            ####################################################
            rowkey, data = self.process_company_hbase_data(item)
            ####################################################
        ...
        with self.hbase_pool.connection() as hbase_conn:
            ...
            #print(rowkey,data)
        logger.info("公司实体HBASE批量数据导入完成,共插入%s条公司记录" % self.hbase_count)
