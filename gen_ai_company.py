...
company_ai = mongo_con[config.get("mongo","info_db")][config.get("mongo","company_label")]
...
company_res = company_ai.find(query)
...
with open("ai_companys.txt","w") as fw:
    fw.write("\n".join(sorted_process_companys))

logger.info("AI企业生成完成，共[{}]家企业".format(len(sorted_process_companys)))
