...
# Filename     : expert_project_relation_mongo.py
# Description  : 科研项目数据添加关联；专家关联
#******************************************************************************
...
class ProjectRelation(object):
    ...
    def query_daily_data(self, crawl_date):
        ...
        res = self.kb_expert_project.find({"update_time": {"$gte": iso_date}}).sort([("_id",1)])
        return res
    ...
    def process_relations(self, doc):
        ...
        # 科研项目与专家关联
        project_expert_relation = self.process_expert_relation(doc)
        if project_expert_relation:
            relations.extend(project_expert_relation)
    ...
    def process(self, crawl_date):
        ...
        for doc in tqdm(docs):
            ...
            if relations:
                ...
                self.kb_expert_project.update_one({"_id":doc["_id"]},{"$set":update})
                self.count_update += 1


        logging.info("[{}]科研项目数据关联完毕，共找到科研项目[{}]条，添加关系记录[{}]条".format(
