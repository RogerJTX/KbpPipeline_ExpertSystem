...
#******************************************************************************
# Author       : jtx
# Last modified: 2020-04-26 14:53
# Filename     : patent_pipeline_process.py
# Description  : 专利实体；从分区中读取数据调用pipeline处理，并存入HBASE结构化文档库；每天跑数据
#******************************************************************************
...
class EntityPipeline(object):
    ...
    def process_patent_hbase_data(self,item):
        ''' 将读取的arango记录转换成hbase存储结构'''

        classid = self.news_class_ids['专利']
        ...
        if "crawl_time" in item["properties"]:
            crawl_t = item["properties"]["crawl_time"]
        column_family = {#专利暂无别名 默认空
    ...
    def link_arangoDb(self, date_str, end_date=""):
        ...
        logging.info("正在处理[专利] arango数据...")
        query_time = ""
        colletion_name = "kb_patent"
        ...
        if end_date and 3== len(end_date.split("-")):
            aql_arango = "FOR entity IN %s FILTER entity.update_time >= '%s' and entity.update_time < '%s' return entity" % (colletion_name,query_time,iso_end_date)
        else:
        ...
        try:
            ...
            '''没有查到数据时，fetch_list抛出异常'''
            query = []
            logger.warn("Arango专利库没有查到数据",e)

        logger.info("Arango库共含有[专利]%s条记录，本次查询%s条记录！" % (collection.figures()['count'],len(query)))
        num = 0
        num_ins = 0
        pre_send_rowkey = []
        pre_send_data = []
        ...
        for item in query:
            num += 1
            ####################################################
            rowkey, data = self.process_patent_hbase_data(item)#
            ####################################################
        ...
        with self.hbase_pool.connection() as hbase_conn:
            ...
            #print(rowkey,data)
        logger.info("专利实体HBASE批量数据导入完成,共插入%s条专利记录" % self.hbase_count)
