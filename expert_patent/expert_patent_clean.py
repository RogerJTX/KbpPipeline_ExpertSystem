...
# Filename     : expert_patent_clean.py
# Description  : 专家成果专利信息清洗
#******************************************************************************
...
class PatentClean(object):
    ...
    def __init__(self):
        ...
        self.res_kb_expert_patent = self.mongo_con[self.config.get("mongo","res_kb_db")][self.config.get("mongo","res_kb_expert_resource")] 
    ...
    def insert_batch(self, batch_data):
        ...
        for data in batch_data:
            ...
            if (not data["publish_code"]) or (not data["publish_date"]):
                self.count_ignore += 1
                logging.info("跳过专利[{}]，缺少有效信息。专利名=[{}]".format(data["_id"],data["title"]))
                continue
        ...
        dupl_datas = self.res_kb_process_expert_patent.find({"publish_code":{"$in":public_codes}})
        ...
        insert_res = self.res_kb_process_expert_patent.insert_many(new_batch)
    ...
    def query_daily_data(self, crawl_date):
        ...
        res = self.res_kb_expert_patent.find({"resourceCode":"P", "crawl_time": {"$gte": iso_date}}).sort([("_id",1)])
        return res
    ...
    def process(self, crawl_date):
        ...
        for doc in tqdm(docs):
            ...
            clean_patent = {
                ...
                "type": self.process_str(doc["type"]),
                "apply_code": self.process_str(doc["applicationNo"]),
                "publish_code": self.process_str(doc["patentNo"]),
                "apply_date": self.process_str(doc["applicationDate"]),
                "publish_date": self.process_str(doc["publicationDate"]),
            ...
            authors = self.process_authors(clean_patent, doc)
            clean_patent.update({"authors": authors})
        ...
        logging.info("[{}]专利数据清洗完毕，共找到专利[{}]条，跳过无专利号的数据[{}]条，批内去重[{}]条，已存在数据[{}]条，清洗库入库[{}]条".format(
