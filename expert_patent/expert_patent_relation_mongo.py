...
# Filename     : expert_patent_relation_mongo.py
# Description  : 专利数据添加关联；专家关联
#******************************************************************************
...
def set_log():
    logging.basicConfig(level=logging.INFO) 
    file_log_handler = RotatingFileHandler(os.path.join(dir_path,"expert_patent_rel_log.txt"), maxBytes=1024 * 1024 * 300, backupCount=10)
...
class PatentRelation(object):
    ...
    def query_daily_data(self, crawl_date):
        ...
        res = self.kb_expert_patent.find({"update_time": {"$gte": iso_date}}).sort([("_id",1)])
        return res
    ...
    def process_relations(self, doc):
        ...
        # 专利与专家关联
        patent_expert_relation = self.process_expert_relation(doc)
        if patent_expert_relation:
            relations.extend(patent_expert_relation)
    ...
    def process(self, crawl_date):
        ...
        for doc in tqdm(docs):
            ...
            if relations:
                ...
                self.kb_expert_patent.update_one({"_id":doc["_id"]},{"$set":update})
                self.count_update += 1


        logging.info("[{}]专利数据关联完毕，共找到专利[{}]条，添加关系记录[{}]条".format(
