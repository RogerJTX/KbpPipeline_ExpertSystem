...
# ******************************************************************************
# Author       : jtx
# Last modified: 2020-10-12 17:09
# Filename     : expert_patent_kId_clean.py
# Description  : 专利共同作者添加KID
# ******************************************************************************
import logging
import requests
...
def set_log():
    logging.basicConfig(level=logging.INFO)
    file_log_handler = RotatingFileHandler(os.path.join(dir_path, "expert_patent_kid_log.txt"),
...
class PatentClean(object):
    ...
    def __init__(self):
        ...
        self.res_kb_process_expert_patent = self.mongo_con[self.config.get("mongo", "res_kb_process")][
            self.config.get("mongo", "process_expert_patent")]  # 专利清洗库
    ...
    def process_kId(self, author):
        ...
        # 查找库中已有数据
        exist_data = self.res_kb_process_expert_patent.find_one({"authors": {
    ...
    def query_daily_data(self, crawl_date):
        ...
        res = self.res_kb_process_expert_patent.find({"crawl_time": {"$gte": iso_date}, "authors.kId": ""}).sort(
            [("_id", 1)])
        return res
    ...
    def process(self, crawl_date):
        ...
        for doc in tqdm(docs):
            ...
            result = self.res_kb_process_expert_patent.update_one({"_id": doc["_id"]},
                                                                  {"$set": update})
            self.count_update += result.modified_count

        logging.info("[{}]专利数据KID添加完毕，共找到专利[{}]条，更新[{}]条".format(
