...
class ProductClean(object):
    ...
    def __init__(self):
        ...
        self.res_kb_product = self.mongo_con[self.config.get("mongo","res_kb_db")]["res_kb_product_website"]
    ...
    def process_desc(self, doc):
        res = ""
        if "desc" in doc and doc["desc"]:
            for item in doc["desc"]:
                res += self.process_str(item)
        return res 
    ...
    def process(self, crawl_date):
        ...
        #     for doc in docs: 
            ...
            logging.info("正在处理产品，产品名=[{}]，ObjectId=[{}]".format(doc["name"], str(doc["_id"])))


            clean_product = {
                "_id": str(doc["_id"]), 
                "name": self.process_str(doc["name"]),
                "introduction": self.process_desc(doc),
                "company_name": self.process_str(doc["manufacturer"]),
                "url": doc["website"] if doc["website"] else "",
                "product_url": doc["url"] if doc["url"] else "",
