...
# Filename     : park_kbp.py
# Description  : 园区信息从构建库转移到arango
#******************************************************************************
...
class KbpPipeline(object):
    def __init__(self):
        ...
        self.res_kb_process_park = self.mongo_con[self.config.get("mongo","res_kb_process")][self.config.get("mongo","process_park")]
        ...
        self.kb_park = self.arango_con[self.config.get("arango","db")][self.config.get("arango","kb_park")]
    ...
    def insert_graph_kb(self, kf):
        ...
        try:
            doc = self.kb_park.createDocument(kf)
            doc.save()
            logger.info("Arango园区库新增完成，园区名=[{}]".format(kf["name"]))
            self.count_graph_insert += 1

        except Exception as e:
            logger.error("新增园区信息至arango失败，园区名=[{}]，信息=[{}]".format(kf["name"], kf), e) 

    def update_graph_kb(self, kf, up_key):

        try:
            doc = self.kb_park[kf['_key']]
    ...
    def query_process(self, process_date, end_date=""):
        ...
        res = self.res_kb_process_park.find(mg_cmd_str).sort([("_id",1)])
        #res = self.res_kb_process_park.find()
    ...
    def process(self, date_str, end_date=""):
        ...
        parks = self.query_process(date_str, end_date)
        ...
        for park in parks:
            ...
            logger.info("处理园区[{}]".format(park["name"]))

            # arango现存园区查询
            exist = self.kb_park.fetchFirstExample( {"_key":park["_id"]}  )

            if not exist:

                logger.info("Arango新增园区[{}]".format(park["name"]))

                properties = self.process_properties(park)
                ...
                logger.info("园区数据已存在图数据库中，园区名=[{}]，园区ID=[{}]".format(park["name"], park["_id"])) 
                self.count_graph_exist += 1

            count += 1

        if count % 100 == 0 or count == self.total:
            logger.info("清洗库前[{}]条园区数据导入园区知识库处理完成".format(count))

        self.close_connection() 

        logger.info("日期[{}]共找到清洗库园区数据[{}]条，arango园区库新增数据{}条，arango已存在园区数据[{}]条".format(
