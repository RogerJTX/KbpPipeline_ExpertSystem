...
#******************************************************************************
# Author       : jtx
# Last modified: 2020-04-27 15:01
# Filename     : park_arango2hbase.py
# Description  : 园区实体；从分区中读取数据调用pipeline处理，并存入HBASE结构化文档库；每天跑数据
#******************************************************************************
...
class EntityPipeline(object):
    ...
    def process_park_hbase_data(self,item):
        ''' 将读取的arango记录转换成hbase存储结构'''

        classid = self.news_class_ids['园区']
    ...
    def link_arangoDb(self, date_str):
        ...
        logging.info("正在处理[园区] arango数据...")
        query_time = ""
        colletion_name = "kb_park"
        ...
        try:
            ...
            query = []
            logger.warn("Arango[园区] 库没有查到数据",e)

        logger.info("HBASE共含有[园区] %s条记录，本次查询%s条记录！" % (collection.figures()['count'],len(query)))
        num = 0
        num_ins = 0
        pre_send_rowkey = []
        pre_send_data = []
        ...
        for item in query:
            num += 1
            ####################################################
            rowkey, data = self.process_park_hbase_data(item)#
            ####################################################
        ...
        with self.hbase_pool.connection() as hbase_conn:
            ...
            #print(rowkey,data)
        logger.info("HBASE批量数据导入完成,共插入%s条园区记录" % self.hbase_count)
