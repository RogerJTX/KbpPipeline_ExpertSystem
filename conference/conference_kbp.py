...
# Filename     : conference_kbp.py
# Description  : 会议信息从构建库转移到arango
#******************************************************************************
...
class KbpPipeline(object):
    def __init__(self):
        ...
        self.res_kb_process_conference = self.mongo_con[self.config.get("mongo","res_kb_process")][self.config.get("mongo","process_conference")]
        ...
        self.kb_conference = self.arango_con[self.config.get("arango","db")][self.config.get("arango","kb_conference")]
    ...
    def insert_graph_kb(self, kf):
        ...
        try:
            doc = self.kb_conference.createDocument(kf)
            doc.save()
            logger.info("Arango会议库新增完成，会议名=[{}]".format(kf["name"]))
            self.count_graph_insert += 1

        except Exception as e:
            logger.error("新增会议信息至arango失败，会议名=[{}]，信息=[{}]".format(kf["name"], kf), e) 

    def update_graph_kb(self, kf, up_key):

        try:
            doc = self.kb_conference[kf['_key']]
    ...
    def query_process(self, process_date, end_date=""):
        ...
        res = self.res_kb_process_conference.find(mg_cmd_str).sort([("_id",1)])
        #res = self.res_kb_process_conference.find({}).sort([("crawl_time",1)])
    ...
    def process(self, date_str, end_date=""):
        ...
        conferences = self.query_process(date_str, end_date)

        count = 0

        # arango数据库会议信息处理
        for conference in conferences:
            logger.info("处理会议[{}]".format(conference["name"]))

            # arango现存会议查询
            exist = self.kb_conference.fetchFirstExample( {"_key":conference["_id"]}  )

            if not exist:

                logger.info("Arango新增会议[{}]".format(conference["name"]))

                properties = self.process_properties(conference)
                ...
                logger.info("会议数据已存在图数据库中，会议名=[{}]，会议ID=[{}]".format(conference["name"], conference["_id"])) 
                self.count_graph_exist += 1

            count += 1

        if count % 100 == 0 or count == self.total:
            logger.info("清洗库前[{}]条会议数据导入会议知识库处理完成".format(count))

        self.close_connection() 

        logger.info("日期[{}]共找到清洗库会议数据[{}]条，arango会议库新增数据{}条，arango已存在会议数据[{}]条".format(
